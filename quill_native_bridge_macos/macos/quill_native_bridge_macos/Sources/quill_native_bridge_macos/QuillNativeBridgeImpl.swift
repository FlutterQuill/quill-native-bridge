...
class QuillNativeBridgeImpl: QuillNativeBridgeApi  {
    ...
    func openGalleryApp() throws {
        guard let url = URL(string: "photos://") else {
            throw PigeonError(code: "INVALID_URL", message: "The URL scheme is invalid.", details: "Unable to create a URL for 'photos://'.")
        }
        ...
        guard canOpen else {
            throw PigeonError(code: "CANNOT_OPEN_URL", message: "Cannot open the Photos app.", details: "The desktop may not have the Photos app installed or it may not support the URL scheme.")
        }
    ...
    func saveImageToGallery(imageBytes: FlutterStandardTypedData, name: String, extension: String, albumName: String?, completion: @escaping (Result<Void, any Error>) -> Void) {
        ...
        guard #available(macOS 10.15, *) else {
            completion(.failure(PigeonError(
                ...
                details: "The current macOS version is: \(macOSVersion.majorVersion).\(macOSVersion.minorVersion).\(macOSVersion.patchVersion)"
            )))
            return
        }
        ...
        guard let infoPlist = Bundle.main.infoDictionary,
            ...
            completion(.failure(PigeonError(
                ...
                message: "The macOS `Info.plist` file has not been configured. The key `\(permissionKey)` is not set.",
                details: nil
            )))
            return
        }
        ...
        savePanel.begin { result in
            ...
            do {
                ...
                completion(.failure(PigeonError(
                    ...
                    details: "An error occurred while writing the image to \(selectedUrl.path). Error: \(error.localizedDescription)"
                )))
            }
        }
